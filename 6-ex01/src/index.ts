let ships: { name: string, pilot: string, crewLimit: number, crew: string[], inMission: boolean }[] = []
...
function addCrewMember(member: string, spaceship: { name: string, crew: string[], crewLimit: number }) {
...
function listRegisteredShips() {
    ...
    const shipsString = ships.reduce((accum: string, ship: { name: string, pilot: string, crewLimit: number, crew: any[], inMission: boolean }, indexof) => {

        const crew = ship.crew.reduce((accum: string, member: string, indexof) => {
            if (indexof === 0) {
                return accum + `${member}`
            } else {
                return accum + `, ${member}`
...
function menu() {
    ...
    switch (option) {
        ...
        case '2':
            ...
            let newMemberShip: { name: string, crew: string[], crewLimit: number }
            do {            
        ...
        case '3':
            let shipTomission: { name: string, crewLimit: number, crew: string[], inMission: Boolean }
...
function getSpaceship(name: string) {
    const spaceship = ships.find((ship: { name: string }) => ship.name === name)
